...
def detect_badge_presence(img_bgr, date_bbox, debug=False):
    ...
    # Ограниченная полоса для конкретной даты
    # Badge близко к дате, но может быть чуть правее
    y1 = max(0, int(y - 1.0*h))  # немного сверху для захвата верхней части badge
    y2 = min(H, int(y + 2.0*h))  # немного снизу для захвата нижней части
    x1 = max(0, int(x + 0.5*w))  # начинаем с середины даты
    x2 = min(W, int(x + w + w*5))  # до 5x ширины даты справа (компромисс)
